...
import { Play, Plus, ListVideo, Trash2, X, Check, Pencil, ChevronUp, ChevronDown, GripVertical } from 'lucide-react'
...
export default function HomePage() {
  ...
  useEffect(() => {
    Promise.all([loadVideos(), loadPlaylists()])
      .then(([v, p]) => {
        setVideos(v)
        setPlaylists(p)
      })
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex flex-1 min-h-0">
        {/* Sidebar */}
        <aside className="w-56 shrink-0 border-r border-border bg-card flex flex-col py-3 gap-0.5">
          <SidebarItem
            label="All Videos"
            count={videos.length}
          ...
          {visibleVideos.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 gap-3 text-center">
              ...
              <p className="text-muted-foreground text-sm">
                {selectedPlaylist ? 'Playlist này chưa có video.' : 'Chưa có video nào.'}
              </p>
              {!selectedPlaylist && (
                <Link
                  to="/admin/new-video"
...
function PlaylistEditModal({
  ...
  function handleSave() {
    ...
    if (!trimmed) return
    onSave({ ...playlist, name: trimmed, videoIds })
  }
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-xl shadow-2xl flex flex-col max-h-[80vh]">
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2">
          ...
          {playlistVideos.map((video, idx) => (
            <div
              key={video.videoId}
              className="flex items-center gap-2 py-2 px-2 rounded-lg hover:bg-accent/50 group"
            >
              ...
              <div className="flex flex-col shrink-0">
                <button
                  onClick={() => move(idx, -1)}
                  disabled={idx === 0}
                  className="w-5 h-5 flex items-center justify-center rounded hover:bg-accent disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
                >
              ...
              <button
                ...
                className="w-6 h-6 flex items-center justify-center rounded hover:bg-destructive/10 hover:text-destructive text-muted-foreground transition-colors shrink-0"
              >
                <X className="w-3.5 h-3.5" />
              </button>
            </div>
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-border shrink-0">
          <button
            onClick={onClose}
            className="h-8 px-4 rounded-lg border border-border text-xs font-medium hover:bg-accent transition-colors"
          >
            Cancel
          </button>
          <button
